...
public class AssetScrapController {
    @Autowired
    AssetScrapService assetScrapService;
    @Autowired
    ...
    public IPage<BatchCheck> list(@RequestBody Map<String, Object> paramMap) {
        ...
        wrapper.eq(BatchCheck::getMenuName, "报废申请").orderByDesc(BatchCheck::getId);
...
//        wrapper.like(ProviderScore1::getLoginName, user.getLoginName()).orderByDesc(ProviderScore1::getId);
    ...
    public BatchCheck get(Integer id) {
        ...
        List<AssetScrap> list = assetScrapService.list(new LambdaQueryWrapper<AssetScrap>().eq(AssetScrap::getBatchId, id));
        //文件
        List<Integer> idList = list.stream().map(AssetScrap::getId).collect(Collectors.toList());
        List<AssetFile> assetFileList = assetFileService.list(new LambdaQueryWrapper<AssetFile>().eq(AssetFile::getType,"AssetScrap").in(AssetFile::getAssetId, idList));
        if (ObjectUtil.isNotEmpty(assetFileList)) {
            for (AssetScrap assetScrap : list) {
                List<FileVO> fileList = new ArrayList<>();
                for (AssetFile assetFile : assetFileList) {
                    if (assetScrap.getId().equals(assetFile.getAssetId())) {
                ...
                if (ObjectUtil.isNotEmpty(fileList)) {
                    assetScrap.setFileList(fileList);
                }
            }
        }
    ...
    public boolean btnHandle(@RequestBody AssetScrapAfter after) {
        return assetScrapService.btnHandle(after, "报废申请");
    }
    @PostMapping("dialogList")
    public List<AssetScrap> dialogList(@RequestBody Map<String, Object> paramMap) {
